...
class StoryParsingTest(unittest.TestCase):
    def test_if_story_contains_valid_date_then_event_should_be_extracted(self):
        ...
        self.assertEqual(len(suggestion.dateTimesCet), 1)
    ...
    def test_if_story_contains_some_valid_dates_then_associated_events_should_be_extracted(self):
        ...
        suggestion = get_suggestion_for_story(story, current_datetime=datetime.datetime(datetime.datetime.now().year, 9, 1, 0, 0, 0), country_data=country_data)
        self.assertEqual(len(suggestion.dateTimesCet), 3)
    ...
    def test_if_story_contains_incomplete_month_date_then_should_default_to_last_day_of_month(self):
        country_data = {
            'eventName': 'Dora',
            'watchLink': 'https://hrti.hrt.hr',
            'stages': ['Semi-final...', 'Final'],
            "altEventNames": []
        }
        story = Story(
    ...
    def test_full_article_non_regression_testing_1(self):
        ...
        suggestion = get_suggestion_for_story(story, current_datetime=datetime.datetime(datetime.datetime.now().year, 9, 1, 0, 0, 0), country_data=country_data)
        self.assertEqual(len(suggestion.dateTimesCet), 7)
        self.assertTrue(suggestion.dateTimesCet[0]['dateTimeCet'][5:10] == "09-30")
        self.assertTrue(suggestion.dateTimesCet[1]['dateTimeCet'][5:10] == "02-06")
        self.assertTrue(suggestion.dateTimesCet[2]['dateTimeCet'][5:10] == "02-13")
        self.assertTrue(suggestion.dateTimesCet[3]['dateTimeCet'][5:10] == "02-20")
        self.assertTrue(suggestion.dateTimesCet[4]['dateTimeCet'][5:10] == "02-27")
        self.assertTrue(suggestion.dateTimesCet[5]['dateTimeCet'][5:10] == "03-06")
        self.assertTrue(suggestion.dateTimesCet[6]['dateTimeCet'][5:10] == "03-13")
    ...
    def test_full_article_non_regression_testing_2(self):
        ...
        suggestion = get_suggestion_for_story(story, current_datetime=datetime.datetime(datetime.datetime.now().year, 9, 1, 0, 0, 0), country_data=country_data)
        self.assertEqual(len(suggestion.dateTimesCet), 6)
        self.assertTrue(suggestion.dateTimesCet[2]['dateTimeCet'][5:10] == "02-02")
        self.assertTrue(suggestion.dateTimesCet[3]['dateTimeCet'][5:10] == "02-09")
        self.assertTrue(suggestion.dateTimesCet[4]['dateTimeCet'][5:10] == "02-16")
        self.assertTrue(suggestion.dateTimesCet[5]['dateTimeCet'][5:10] == "02-23")
    ...
    def test_full_article_non_regression_testing_3(self):
        ...
        story = Story("Finland", ".Six acts have been selected to compete in UMK 2020, as YLE announces the total number of submission received for the competition..After assessing a total of 426 submissions into UMK 2020, YLE has selected the six finalists who will be competing to represent Finland at Eurovision 2020. A jury formed of ten music professionals both inside and outside of YLE selected the six finalists..The six acts who will be competing in UMK 2020 will be revealed on January 21 live on the YLE website. UMK will be held on March 7 at the Mediapolis Studio in Tampere. The selected songs, as well the the jury members who chose them will also be revealed on January 21..This is the first time since the 2017 contest that Finland has opened UMK back up to any interested singers and songwriters. Yle had opted to internally select their singer for Eurovision, and then use UMK as a song selection show..Finland was represented by Darude feat. Sebastian Rejman with the song “Look Away” in Tel Aviv. Finland finished last in their semi-final scoring just 23 points. Finland received 14 points from the televote and 9 points from the juries.", "https://eurovoix.com")
        suggestion = get_suggestion_for_story(story, current_datetime=datetime.datetime(datetime.datetime.now().year, 9, 1, 0, 0, 0), country_data=country_data)
        self.assertEqual(len(suggestion.dateTimesCet), 2)
    ...
    def test_if_xml_items_include_items_with_national_selection_category_then_should_return_them(self):
        ...
        self.assertEqual(len(nf_items), 2)
    ...
    def test_if_xml_items_include_items_without_national_selection_category_but_with_nf_name_in_title_then_should_return_them(self):
        ...
        nf_items = get_nf_items_from_xml_items(items, nf_names=['melodifestivalen', 'supernova'])
        titles = list(map(lambda i: i.find('title').text, nf_items))
        self.assertEqual(len(nf_items), 2)
    ...
    def test_if_xml_items_include_items_with_unwanted_categories_then_should_not_return_them(self):
        ...
        nf_items = get_nf_items_from_xml_items(items, nf_names=['melodifestivalen'])
        titles = list(map(lambda i: i.find('title').text, nf_items))
        self.assertEqual(len(nf_items), 0)
    ...
    def test_if_xml_items_include_items_without_national_selection_category_but_with_nf_name_or_alt_name_in_title_then_should_return_them(self):
        ...
        nf_items = get_nf_items_from_xml_items(items, nf_names=['melodifestivalen', 'melfest', 'umk', 'uuden musiikin kilpailu'])
        titles = list(map(lambda i: i.find('title').text, nf_items))
        self.assertEqual(len(nf_items), 2)
